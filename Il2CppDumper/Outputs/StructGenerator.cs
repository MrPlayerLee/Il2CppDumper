...
namespace Il2CppDumper
{
    public class StructGenerator
    {
        ...
        public void WriteScript(string outputDir)
        {
            ...
            {
                if (il2Cpp.methodPointers != null) orderedPointers.AddRange(il2Cpp.methodPointers);
            }
            if (il2Cpp.genericMethodPointers != null) orderedPointers.AddRange(il2Cpp.genericMethodPointers);
            if (il2Cpp.invokerPointers != null) orderedPointers.AddRange(il2Cpp.invokerPointers);
            if (il2Cpp.Version < 29 && executor.customAttributeGenerators != null)
            {
            ...
            if (il2Cpp.Version >= 27)
            {
                ...
                if (sectionHelper != null)
                {
                    ...
                    {
                        // [!] 순수 ulong 타입만 사용하여 연산 오류 원천 차단
                        il2Cpp.Position = sec.offset;
                        ulong end = Math.Min(sec.offsetEnd, il2Cpp.Length) - il2Cpp.PointerSize;
                                        ...
                                        if (va > 0)
                                        {
                                            switch ((Il2CppMetadataUsage)usage)
                                            {
                                                case Il2CppMetadataUsage.kIl2CppMetadataUsageTypeInfo:
                                                    if (decodedIndex < (uint)il2Cpp.types.Length)
            ...
            var stringLiterals = json.ScriptString.Select(x => new { value = x.Value, address = $"0x{x.Address:X}" }).ToArray();
            ...
            for (int i = 0; i < genericClassList.Count; i++) AddGenericClassStruct(genericClassList[i]);
            foreach (var info in structInfoList) structInfoWithStructName[info.TypeName + "_o"] = info;
            var headerStruct = new StringBuilder();
            foreach (var info in structInfoList) headerStruct.Append(RecursionStructInfo(info));
            var sb = new StringBuilder();
            sb.Append(HeaderConstants.GenericHeader);
            switch (il2Cpp.Version)
            {
                case 22: sb.Append(HeaderConstants.HeaderV22); break;
                case 23: case 24: sb.Append(HeaderConstants.HeaderV240); break;
                case 24.1: sb.Append(HeaderConstants.HeaderV241); break;
                case 24.2: case 24.3: case 24.4: case 24.5: sb.Append(HeaderConstants.HeaderV242); break;
                case 27: case 27.1: case 27.2: sb.Append(HeaderConstants.HeaderV27); break;
                case 29: case 29.1: case 31: sb.Append(HeaderConstants.HeaderV29); break;
                default: Console.WriteLine($"WARNING: Version [{il2Cpp.Version}] not supported for .h"); return;
            }
        ...
        private void AddMetadataUsageTypeInfo(ScriptJson json, uint index, ulong address)
        {
            ...
            var scriptMetadata = new ScriptMetadata { Address = il2Cpp.GetRVA(address), Name = typeName + "_TypeInfo" };
            json.ScriptMetadata.Add(scriptMetadata);
            var signature = GetIl2CppStructName(type);
            scriptMetadata.Signature = signature.EndsWith("_array") ? "Il2CppClass*" : FixName(signature) + "_c*";
        }
        ...
        private void AddMetadataUsageIl2CppType(ScriptJson json, uint index, ulong address)
        {
            var type = il2Cpp.types[index];
            var typeName = executor.GetTypeName(type, true, false);
            var scriptMetadata = new ScriptMetadata { Address = il2Cpp.GetRVA(address), Name = typeName + "_var", Signature = "Il2CppType*" };
            json.ScriptMetadata.Add(scriptMetadata);
        }
        ...
        private void AddMetadataUsageMethodDef(ScriptJson json, uint index, ulong address)
        {
            ...
            var scriptMetadataMethod = new ScriptMetadataMethod { Address = il2Cpp.GetRVA(address), Name = "Method$" + methodName };
            json.ScriptMetadataMethod.Add(scriptMetadataMethod);
            var imageName = typeDefImageNames[typeDef];
            var methodPointer = il2Cpp.GetMethodPointer(imageName, methodDef);
            if (methodPointer > 0) scriptMetadataMethod.MethodAddress = il2Cpp.GetRVA(methodPointer);
        }
        ...
        private void AddMetadataUsageFieldInfo(ScriptJson json, uint index, ulong address)
        {
            ...
            var scriptMetadata = new ScriptMetadata { Address = il2Cpp.GetRVA(address), Name = "Field$" + fieldName };
            json.ScriptMetadata.Add(scriptMetadata);
        }

        private void AddMetadataUsageStringLiteral(ScriptJson json, uint index, ulong address)
        {
            var scriptString = new ScriptString { Address = il2Cpp.GetRVA(address), Value = metadata.GetStringLiteralFromIndex(index) };
        ...
        private void AddMetadataUsageMethodRef(ScriptJson json, uint index, ulong address)
        {
            ...
            var scriptMetadataMethod = new ScriptMetadataMethod { Address = il2Cpp.GetRVA(address) };
            json.ScriptMetadataMethod.Add(scriptMetadataMethod);
            (var methodSpecTypeName, var methodSpecMethodName) = executor.GetMethodSpecName(methodSpec, true);
            scriptMetadataMethod.Name = "Method$" + methodSpecTypeName + "." + methodSpecMethodName + "()";
            if (il2Cpp.methodSpecGenericMethodPointers.TryGetValue(methodSpec, out var genericMethodPointer) && genericMethodPointer > 0)
                scriptMetadataMethod.MethodAddress = il2Cpp.GetRVA(genericMethodPointer);
        }

        private static string FixName(string str)
        {
            if (keyword.Contains(str)) str = "_" + str;
            else if (specialKeywords.Contains(str)) str = "_" + str + "_";
            return Regex.IsMatch(str, "^[0-9]") ? "_" + str : Regex.Replace(str, "[^a-zA-Z0-9_]", "_");
        }
        ...
        private string ParseType(Il2CppType il2CppType, Il2CppGenericContext context = null)
        {
            switch (il2CppType.type)
            {
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_PTR: return ParseType(il2Cpp.GetIl2CppType(il2CppType.data.type)) + "*";
                case Il2CppTypeEnum.IL2CPP_TYPE_VALUETYPE:
                    var typeDef = executor.GetTypeDefinitionFromIl2CppType(il2CppType);
                    return typeDef.IsEnum ? ParseType(il2Cpp.types[typeDef.elementTypeIndex]) : structNameDic[typeDef] + "_o";
                case Il2CppTypeEnum.IL2CPP_TYPE_CLASS: return structNameDic[executor.GetTypeDefinitionFromIl2CppType(il2CppType)] + "_o*";
                case Il2CppTypeEnum.IL2CPP_TYPE_VAR:
                    if (context != null && context.class_inst != 0)
                    {
                        ...
                        var pointers = il2Cpp.MapVATR<ulong>(genericInst.type_argv, (uint)genericInst.type_argc);
                        return ParseType(il2Cpp.GetIl2CppType(pointers[genericParameter.num]));
                    }
                    return "Il2CppObject*";
                case Il2CppTypeEnum.IL2CPP_TYPE_ARRAY:
                    ...
                    var typeStructName = GetIl2CppStructName(elementType, context) + "_array";
                    if (structNameHashSet.Add(typeStructName)) ParseArrayClassStruct(elementType, context);
                    return typeStructName + "*";
                case Il2CppTypeEnum.IL2CPP_TYPE_GENERICINST:
                    var genericClass = il2Cpp.MapVATR<Il2CppGenericClass>(il2CppType.data.generic_class);
                    var gTypeDef = executor.GetGenericClassTypeDefinition(genericClass);
                    var gStructName = genericClassStructNameDic[il2CppType.data.generic_class];
                    if (structNameHashSet.Add(gStructName)) genericClassList.Add(il2CppType.data.generic_class);
                    return (gTypeDef.IsValueType && !gTypeDef.IsEnum) ? gStructName + "_o" : gStructName + "_o*";
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_SZARRAY:
                    var szElementType = il2Cpp.GetIl2CppType(il2CppType.data.type);
                    var szStructName = GetIl2CppStructName(szElementType, context) + "_array";
                    if (structNameHashSet.Add(szStructName)) ParseArrayClassStruct(szElementType, context);
                    return szStructName + "*";
                case Il2CppTypeEnum.IL2CPP_TYPE_MVAR:
                    if (context != null && context.method_inst != 0)
                    {
                        var genericParameter = executor.GetGenericParameteFromIl2CppType(il2CppType);
        ...
        {
            var structInfo = new StructInfo { TypeName = structNameDic[typeDef], IsValueType = typeDef.IsValueType };
        ...
        private void AddGenericClassStruct(ulong pointer)
        {
            ...
            var typeDef = executor.GetGenericClassTypeDefinition(genericClass);
            var structInfo = new StructInfo { TypeName = genericClassStructNameDic[pointer], IsValueType = typeDef.IsValueType };
            structInfoList.Add(structInfo);
            AddParents(typeDef, structInfo);
        ...
        private void AddParents(Il2CppTypeDefinition typeDef, StructInfo structInfo)
        {
            if (!typeDef.IsValueType && !typeDef.IsEnum && typeDef.parentIndex >= 0)
            {
                ...
                if (parent.type != Il2CppTypeEnum.IL2CPP_TYPE_OBJECT) structInfo.Parent = GetIl2CppStructName(parent);
            }
        }
        ...
        private void AddFields(Il2CppTypeDefinition typeDef, StructInfo structInfo, Il2CppGenericContext context)
        {
            if (typeDef.field_count > 0)
            {
                ...
                for (var i = typeDef.fieldStart; i < fieldEnd; ++i)
                {
                    ...
                    if ((fieldType.attrs & FIELD_ATTRIBUTE_LITERAL) != 0) continue;
                    var fieldName = FixName(metadata.GetStringFromIndex(fieldDef.nameIndex));
                    if (!cache.Add(fieldName)) fieldName = $"_{i - typeDef.fieldStart}_{fieldName}";
                    var structFieldInfo = new StructFieldInfo { FieldTypeName = ParseType(fieldType, context), FieldName = fieldName, IsValueType = IsValueType(fieldType, context), IsCustomType = IsCustomType(fieldType, context) };
                    if ((fieldType.attrs & FIELD_ATTRIBUTE_STATIC) != 0) structInfo.StaticFields.Add(structFieldInfo);
                    else structInfo.Fields.Add(structFieldInfo);
                }
            }
        }
        ...
        private void AddVTableMethod(StructInfo structInfo, Il2CppTypeDefinition typeDef)
        {
            ...
            for (int i = 0; i < typeDef.vtable_count; i++)
            {
                ...
                Il2CppMethodDefinition methodDef = (usage == 6) ? metadata.methodDefs[il2Cpp.methodSpecs[index].methodDefinitionIndex] : metadata.methodDefs[index];
                if (methodDef.slot != ushort.MaxValue) dic[methodDef.slot] = methodDef;
            }
            if (dic.Count > 0)
            {
                structInfo.VTableMethod = new StructVTableMethodInfo[dic.Last().Key + 1];
                foreach (var i in dic) structInfo.VTableMethod[i.Key] = new StructVTableMethodInfo { MethodName = FixName(metadata.GetStringFromIndex(i.Value.nameIndex)) };
            }
        }
        ...
        private void AddRGCTX(StructInfo structInfo, Il2CppTypeDefinition typeDef)
        {
            ...
            if (collection != null)
            {
                ...
                {
                    var structRGCTXInfo = new StructRGCTXInfo { Type = definitionData.type };
                    structInfo.RGCTXs.Add(structRGCTXInfo);
                    var rgctxDefData = (il2Cpp.Version >= 27.2) ? il2Cpp.MapVATR<Il2CppRGCTXDefinitionData>(definitionData._data) : definitionData.data;
                    switch (definitionData.type)
                    {
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_TYPE: structRGCTXInfo.TypeName = FixName(executor.GetTypeName(il2Cpp.types[rgctxDefData.typeIndex], true, false)); break;
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_CLASS: structRGCTXInfo.ClassName = FixName(executor.GetTypeName(il2Cpp.types[rgctxDefData.typeIndex], true, false)); break;
        ...
        {
            structNameDic.Add(typeDef, GetUniqueName(FixName(executor.GetTypeDefName(typeDef, true, true))));
        }
        ...
        private string GetUniqueName(string name)
        {
            var fixName = name;
            int i = 1;
            while (!structNameHashSet.Add(fixName)) fixName = $"{name}_{i++}";
            return fixName;
        }
        ...
        private string RecursionStructInfo(StructInfo info)
        {
            ...
            {
                if (il2Cpp is PE && !info.IsValueType) sb.Append($"struct __declspec(align({(il2Cpp.Is32Bit ? "4" : "8")})) {info.TypeName}_Fields {{\n");
                else sb.Append($"struct {info.TypeName}_Fields {{\n");
            }
            foreach (var field in info.Fields)
            {
                if (field.IsValueType && structInfoWithStructName.TryGetValue(field.FieldTypeName, out var fieldInfo))
                    pre.Append(RecursionStructInfo(fieldInfo));
                sb.Append(field.IsCustomType ? $"\tstruct {field.FieldTypeName} {field.FieldName};\n" : $"\t{field.FieldTypeName} {field.FieldName};\n");
            }
            sb.Append("};\n");
            if (info.RGCTXs.Count > 0)
            {
                ...
                for (int i = 0; i < info.RGCTXs.Count; i++)
                {
                    ...
                    switch (rgctx.Type)
                    {
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_TYPE: sb.Append($"\tIl2CppType* _{i}_{rgctx.TypeName};\n"); break;
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_CLASS: sb.Append($"\tIl2CppClass* _{i}_{rgctx.ClassName};\n"); break;
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_METHOD: sb.Append($"\tMethodInfo* _{i}_{rgctx.MethodName};\n"); break;
                    }
                }
                sb.Append("};\n");
            }
            if (info.VTableMethod != null && info.VTableMethod.Length > 0)
            {
                sb.Append($"struct {info.TypeName}_VTable {{\n");
                for (int i = 0; i < info.VTableMethod.Length; i++) sb.Append($"\tVirtualInvokeData _{i}_{info.VTableMethod[i]?.MethodName ?? "unknown"};\n");
                sb.Append("};\n");
            }
            sb.Append($"struct {info.TypeName}_c {{\n\tIl2CppClass_1 _1;\n");
            sb.Append(info.StaticFields.Count > 0 ? $"\tstruct {info.TypeName}_StaticFields* static_fields;\n" : "\tvoid* static_fields;\n");
            sb.Append(info.RGCTXs.Count > 0 ? $"\t{info.TypeName}_RGCTXs* rgctx_data;\n" : "\tIl2CppRGCTXData* rgctx_data;\n");
            sb.Append($"\tIl2CppClass_2 _2;\n");
            sb.Append(info.VTableMethod != null && info.VTableMethod.Length > 0 ? $"\t{info.TypeName}_VTable vtable;\n" : "\tVirtualInvokeData vtable[32];\n");
            sb.Append($"}};\nstruct {info.TypeName}_o {{\n");
        ...
        private string GetIl2CppStructName(Il2CppType il2CppType, Il2CppGenericContext context = null)
        {
            switch (il2CppType.type)
            {
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_OBJECT: return structNameDic[executor.GetTypeDefinitionFromIl2CppType(il2CppType)];
                case Il2CppTypeEnum.IL2CPP_TYPE_PTR: return GetIl2CppStructName(il2Cpp.GetIl2CppType(il2CppType.data.type));
                case Il2CppTypeEnum.IL2CPP_TYPE_ARRAY:
                    var arrayType = il2Cpp.MapVATR<Il2CppArrayType>(il2CppType.data.array);
                    var elementType = il2Cpp.GetIl2CppType(arrayType.etype);
                    var typeStructName = GetIl2CppStructName(elementType, context) + "_array";
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_VAR:
                    if (context != null && context.class_inst != 0)
                    {
                        ...
                        var genericInst = il2Cpp.MapVATR<Il2CppGenericInst>(context.class_inst);
                        var pointers = il2Cpp.MapVATR<ulong>(genericInst.type_argv, (uint)genericInst.type_argc);
                        return GetIl2CppStructName(il2Cpp.GetIl2CppType(pointers[genericParameter.num]));
                    }
        ...
        private bool IsValueType(Il2CppType il2CppType, Il2CppGenericContext context)
        {
            switch (il2CppType.type)
            {
                case Il2CppTypeEnum.IL2CPP_TYPE_VALUETYPE: return !executor.GetTypeDefinitionFromIl2CppType(il2CppType).IsEnum;
                case Il2CppTypeEnum.IL2CPP_TYPE_GENERICINST:
                    var typeDef = executor.GetGenericClassTypeDefinition(il2Cpp.MapVATR<Il2CppGenericClass>(il2CppType.data.generic_class));
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_VAR:
                    if (context != null && context.class_inst != 0)
                    {
                        ...
                        var genericInst = il2Cpp.MapVATR<Il2CppGenericInst>(context.class_inst);
                        var pointers = il2Cpp.MapVATR<ulong>(genericInst.type_argv, (uint)genericInst.type_argc);
                        return IsValueType(il2Cpp.GetIl2CppType(pointers[genericParameter.num]), null);
                    }
                    return false;
                case Il2CppTypeEnum.IL2CPP_TYPE_MVAR:
                    if (context != null && context.method_inst != 0)
        ...
        private bool IsCustomType(Il2CppType il2CppType, Il2CppGenericContext context)
        {
            switch (il2CppType.type)
            {
                case Il2CppTypeEnum.IL2CPP_TYPE_PTR: return IsCustomType(il2Cpp.GetIl2CppType(il2CppType.data.type), context);
                case Il2CppTypeEnum.IL2CPP_TYPE_STRING:
                case Il2CppTypeEnum.IL2CPP_TYPE_CLASS:
                case Il2CppTypeEnum.IL2CPP_TYPE_ARRAY:
                ...
                case Il2CppTypeEnum.IL2CPP_TYPE_VALUETYPE:
                    var typeDef = executor.GetTypeDefinitionFromIl2CppType(il2CppType);
                    return typeDef.IsEnum ? IsCustomType(il2Cpp.types[typeDef.elementTypeIndex], context) : true;
                case Il2CppTypeEnum.IL2CPP_TYPE_GENERICINST:
                    var gTypeDef = executor.GetGenericClassTypeDefinition(il2Cpp.MapVATR<Il2CppGenericClass>(il2CppType.data.generic_class));
                    return gTypeDef.IsEnum ? IsCustomType(il2Cpp.types[gTypeDef.elementTypeIndex], context) : true;
                case Il2CppTypeEnum.IL2CPP_TYPE_VAR:
                    if (context != null && context.class_inst != 0)
                    {
                        var genericParameter = executor.GetGenericParameteFromIl2CppType(il2CppType);
                        var genericInst = il2Cpp.MapVATR<Il2CppGenericInst>(context.class_inst);
                        var pointers = il2Cpp.MapVATR<ulong>(genericInst.type_argv, (uint)genericInst.type_argc);
                        return IsCustomType(il2Cpp.GetIl2CppType(pointers[genericParameter.num]), null);
                    }
                    return false;
                case Il2CppTypeEnum.IL2CPP_TYPE_MVAR:
                    if (context != null && context.method_inst != 0)
        ...
        private void GenerateMethodInfo(string methodInfoName, string structTypeName, List<StructRGCTXInfo> rgctxs)
        {
            if (rgctxs.Count > 0)
            {
                ...
                for (int i = 0; i < rgctxs.Count; i++)
                {
                    ...
                    switch (rgctx.Type)
                    {
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_TYPE: methodInfoHeader.Append($"\tIl2CppType* _{i}_{rgctx.TypeName};\n"); break;
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_CLASS: methodInfoHeader.Append($"\tIl2CppClass* _{i}_{rgctx.ClassName};\n"); break;
                        case Il2CppRGCTXDataType.IL2CPP_RGCTX_DATA_METHOD: methodInfoHeader.Append($"\tMethodInfo* _{i}_{rgctx.MethodName};\n"); break;
                    }
                }
                methodInfoHeader.Append("};\n");
            }
            methodInfoHeader.Append($"struct {methodInfoName} {{\n\tIl2CppMethodPointer methodPointer;\n");
            if (il2Cpp.Version >= 29) { methodInfoHeader.Append("\tIl2CppMethodPointer virtualMethodPointer;\n\tInvokerMethod invoker_method;\n"); }
            else { methodInfoHeader.Append("\tvoid* invoker_method;\n"); }
            methodInfoHeader.Append("\tconst char* name;\n");
            methodInfoHeader.Append(il2Cpp.Version <= 24 ? $"\t{structTypeName}_c *declaring_type;\n" : $"\t{structTypeName}_c *klass;\n");
            methodInfoHeader.Append("\tconst Il2CppType *return_type;\n");
            methodInfoHeader.Append(il2Cpp.Version >= 29 ? "\tconst Il2CppType** parameters;\n" : "\tconst void* parameters;\n");
            methodInfoHeader.Append(rgctxs.Count > 0 ? $"\tconst {methodInfoName}_RGCTXs* rgctx_data;\n" : "\tconst Il2CppRGCTXData* rgctx_data;\n");
            methodInfoHeader.Append("\tunion\n\t{\n\t\tconst void* genericMethod;\n");
            methodInfoHeader.Append(il2Cpp.Version >= 27 ? "\t\tconst void* genericContainerHandle;\n" : "\t\tconst void* genericContainer;\n");
            methodInfoHeader.Append("\t};\n");
            if (il2Cpp.Version <= 24) methodInfoHeader.Append("\tint32_t customAttributeIndex;\n");
